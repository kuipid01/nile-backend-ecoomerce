...
export class AuthService {
  async createUser(
    req: Request,
    res: Response
  ): Promise<ServiceResponse<IUser | null> | void> {
    ...
    try {
      ...
      const userExist = await prisma.findOne({ email });
      if (userExist) {
        return ServiceResponse.failure(
          "User already exists",
          null,
  ...
  async logUser(
    req: Request,
    res: Response
  ): Promise<void | ServiceResponse<IUser | null>> {
    try {
      ...
      expires: new Date(
        Date.now() +
          (Number(process.env.JWT_COOKIE_EXPIRES_IN) || 90) *
            ...
            60 *
            60 *
            1000 // Fallback to 90 days if env variable is missing
      ),
      httpOnly: true,
  ...
  async protectRoute(
    ...
    res: Response,
    next: NextFunction
  ): Promise<ServiceResponse<null | IUser> | void> {
    let token;
    if (
    ...
    const decoded = (await promisifiedJwt(
      token,
      process.env.JWT_SECRET!
    )) as JwtPayload;
